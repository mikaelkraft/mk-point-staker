...
function mkps_display_user_points_shortcode($atts) {
    $atts = shortcode_atts(['user_id' => 0], $atts, 'mkps_user_points');
...
function mkps_display_user_stakes_shortcode($atts) {
    $atts = shortcode_atts(['user_id' => 0], $atts, 'mkps_user_stakes');
    ...
    echo '<p>' . sprintf(__('Wins: %d | Losses: %d', 'mk-point-staker'), $wins, $losses) . '</p>';
    if ($user_stakes->have_posts()) {
        ...
        while ($user_stakes->have_posts()) {
            ...
            $is_accepted = get_post_meta(get_the_ID(), '_mkps_stake_accepted', true);
            echo '<li class="mkps-stake-item" data-stake-id="' . get_the_ID() . '" data-accepted="' . ($is_accepted ? 1 : 0) . '">' .
                 sprintf(__('Stake for %d points - %s', 'mk-point-staker'), $stake_points, $accepted_by ? __('Accepted', 'mk-point-staker') : __('Pending', 'mk-point-staker')) .
                 '</li>';
        }
        echo '</ul>';
    } else {
...
/**
 * Filter for Custom Profile Manager Integration
 */
...
function mkps_add_stake_count_to_um_menu($menu_items) {
    ...
    if (isset($menu_items['notifications'])) {
        $menu_items['notifications']['title'] .= $available_stakes > 0 ? ' <span class="mkps-notification-bubble">' . $available_stakes . '</span>' : '';
    } elseif (isset($menu_items['stakes'])) {
        $menu_items['stakes']['title'] .= $available_stakes > 0 ? ' <span class="mkps-notification-bubble">' . $available_stakes . '</span>' : '';
    }
